 ...
 *
 * Constructed from a `PID` and two `ExitCondition` objects so the call site
 * reads as the actual mental model — "a controller is a PID plus two exit
 * windows." Internally, the constructor pulls the numeric values out via
 * getters and stores them as flat floats. This means:
 *   - construction is self-documenting (no positional 9-float blob),
 *   - storage is plain data — every field is directly readable and writable
 *     by movement code (`settings.kP`, `settings.smallError`, etc.),
 *   - a route can tune any field at runtime in one line:
 *       chassis.lateralSettings.kP = 15;
 *       chassis.lateralSettings.smallError = 0.5;
 *
...
public:
    ControllerSettings(PID pid, ExitCondition smallExit, ExitCondition largeExit)
        : kP                (pid.getkP()),
          kI                (pid.getkI()),
          kD                (pid.getkD()),
          windupRange       (pid.getWindupRange()),
          smallError        (smallExit.getRange()),
          smallErrorTimeout (smallExit.getTime()),
          largeError        (largeExit.getRange()),
          largeErrorTimeout (largeExit.getTime()) {}
