...
- **WinLib:** Reduced to a single window — flat fields `exitRange` and `exitTimeout`. The constructor takes one `ExitCondition`, not two.
  ...
  ```
  The "looser good-enough at low speed" role is now served entirely by each motion's `earlyExitRange` param field (instantaneous threshold, only fires when `params.minSpeed > 0`).
...
- **WinLib:** `moveFor(float distance, float theta, int timeout, MoveForParams)` takes an **explicit target heading** `theta`. Each tick it runs a second (angular) PID toward `theta` and mixes the two outputs — `left = lateral + angular`, `right = lateral − angular` — so the robot actively steers to the commanded absolute heading while driving the distance. The angular correction is clamped to ±1 V so it can't overpower the drive.
...
### `moveFor` takes a dedicated `MoveForParams` — heading-hold gains (`turnKp`/`turnKd`) are now caller-visible
- **Reference:** N/A — the heading-hold correction is a WinLib addition (the reference lateral move doesn't hold an explicit heading), so there's no upstream counterpart. This entry records a divergence from WinLib's own earlier shape.
- **WinLib:** `moveFor` no longer shares `LateralParams` with `moveToPoint`; it takes its own `MoveForParams { forwards, maxSpeed, minSpeed, earlyExitRange, turnKp, turnKd }`. The PID that keeps the robot on `theta` while driving straight used to be a **hardcoded** `angular_kP = 0.075` that also borrowed `kI`/`kD`/`windupRange` from `angularSettings`. It is now a pure PD built from the caller-visible `params.turnKp` / `params.turnKd` (`angular_PID(turnKp, 0, turnKd, 0)`), fully decoupled from `angularSettings`.
- **Why:** Two problems with the old form. (1) The correction gain was **trapped** inside the library file — tuning it meant editing WinLib, not the route, forcing the team to bounce between files mid-tune. (2) Borrowing `angularSettings` was a category error: those gains are tuned for *in-place turns* (`turnToHeading`/`turnBy`/swings) and are too hot for a gentle straight-line heading trim. A dedicated struct with its own turn gains lets a route tune the heading-hold in one line, and the `turnKp`/`turnKd` naming matches `WallParams`' correction gains so "a turn trim on top of a straight drive" reads the same across every motion that has one.

### `forwards` is an `int` sign (±1), not a `bool` (on the driving motions)
- **Reference:** LemLib/Genesis motions use `bool forwards = true`.
- **WinLib:** On `MoveForParams`, `MoveToPointParams`, and `WallParams`, `forwards` is an `int` holding `+1` (forwards) or `-1` (backwards), used directly as a sign multiplier on the target: `MMTodeg(distance) * params.forwards`. (`AngularParams` and `MoveToPoseParams` keep their `bool forwards`, since there the flag selects a *facing*, not a math sign.)
...
### Dropped: `turnToPoint(x, y, timeout, params)` — not provided
- **Reference:** Genesis/LemLib expose `turnToPoint(x, y, timeout, params)` — turn in place to face an (x, y) coordinate (compute the bearing `atan2(dy, dx)`, then behave like `turnToHeading`).
- **WinLib:** **not provided.** For a while `Chassis::turnToPoint` was *declared* in `chassis.hpp` as a to-do (never implemented), but the declaration has now been removed entirely — WinLib ships no turn-to-point motion.
- **Why:** it earns its place only when a route actually needs "rotate to face this point," and none has. When one does, the one-liner `Chs->turnToHeading(atan2/bearing-to-point, timeout)` computed at the call site covers it without a dedicated method. Keeping an unimplemented declaration around was a teaching trap (students would call a method that silently does nothing), so it's gone until a real need brings it back as an implemented motion. (`swingToPoint` still recomputes a bearing internally — that's a different, implemented motion.)
...
### `moveToPoint`/`moveToPose` steering decoupled from `angularSettings` — `MoveToPointParams` rename + `turnKp`/`turnKd`
- **Reference:** LemLib's `moveToPoint`/`moveToPose` run their aim/carrot steering off the angular controller's gains.
- **WinLib:** The aim-at-point (`moveToPoint`) and carrot (`moveToPose`) steering PIDs no longer read `angularSettings` directly. Each motion's params now carry `turnKp`/`turnKd` (`std::optional<float>`), and the steering PID is a pure PD — `angular_PID(turnKp.value_or(angularSettings.kP), 0, turnKd.value_or(angularSettings.kD), 0)`. Empty (the default) inherits `angularSettings.kP`/`kD`, so out-of-box behavior is unchanged (on ace, `angularSettings.kI == 0`, so dropping the integral makes it byte-identical). `moveToPoint`'s param struct was also renamed `LateralParams → MoveToPointParams`: now that `moveFor` split off to `MoveForParams`, the point motion is the sole user, so it takes LemLib's actual struct name.
- **Why:** `angularSettings` is tuned for *in-place turns* (`turnToHeading`/`turnBy`/swings) — big, sharp corrections. Borrowed as a steering trim on a robot that's already rolling, those gains overreact and the robot wags/oscillates on approach. Splitting the steering gain out lets a route dial it down without touching turn tuning, while inherit-by-default keeps the common case zero-config. Same trapped-constant fix as `moveFor`'s heading-hold, one level over.
- **Supersedes (kept as history):** the "Reusing `LateralParams`" line in the `moveToPoint` entry above — `moveFor` and `moveToPoint` no longer share a param struct.
...
### `clamp_Signed` — the shared "clamp to maxSpeed, floor to minSpeed" output-shaping helper
- **Reference:** Genesis/LemLib inline the speed-band shaping in every motion: `output = clamp(output, maxSpeed, -maxSpeed);` immediately followed by `if (fabs(output) < fabs(minSpeed)) output = minSpeed * sgn(output);`. The two lines are copy-pasted into each movement file.
- **WinLib:** factored those two steps into one free function `WinLib::clamp_Signed(input, max, min)` in `util.hpp` / `util.cpp` (next to `clamp` and `sgn`, same `(input, max, min)` param order as `clamp`). All 8 motions (`turnToHeading`, `turnBy`, `swingToHeading`, `swingToPoint`, `moveFor`, `moveToPoint`, `moveToPose`, `moveByWall`) now call `clamp_Signed(x, params.maxSpeed, params.minSpeed)` in place of the inlined block. Behavior is byte-for-byte identical (same clamp, same `std::fabs` floor, same `min * sgn(x)` sign handling including `sgn(0) == 0` leaving a zero output at zero).
- **Why:** pure de-duplication + readability. One named helper ("clamp the magnitude into a speed band, keep the sign") replaces the same two cryptic lines repeated in eight files, so a student reads intent instead of decoding mechanics, and the band logic has a single place to change. Not touched: the separate angular `clamp(x, +1, -1)` lines (no minSpeed floor) and the `ratio`-based desaturation in `moveToPose`/`moveToPoint`.
